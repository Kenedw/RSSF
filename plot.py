...
def map(lista_de_host, lista_de_nos, altura, largura): 

	#dimensao x e y varia de 0 a 10
	plt.axis([0, altura, 0, largura])

	#label
	plt.xlabel("Metros")
	plt.ylabel("Metros")

	for host in lista_de_host: 
	...
	for i in range(len(lista_de_nos)-1): # pecorre o vetor com todos os nos, e liga as retas
		x = lista_de_nos[i].coordinates[0] 
		y = lista_de_nos[i].coordinates[1]

		dx = lista_de_nos[i+1].coordinates[0] - lista_de_nos[i].coordinates[0] #diferenca entre pontos
